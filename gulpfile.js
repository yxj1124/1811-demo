...
//	const config=require('./config/index')
	const config=require('./config')//在这里因为取得是index。所以可以省略了
	//压缩html
	...
	const  rename = require('gulp-rename')//设置压缩后的文件名(重命名)
 	//给css增加前缀
...
//		    .pipe(htmlmin(config.htmloptions))//清除注释的
		    .pipe(gulp.dest('./dist'))
	})
	//处理css，合并css  压缩css  前缀 ，输出
	gulp.task('handle:css',function(){
	 let streams=[] //村放下面多个文件流的数组
		...
		for(const file in config.cssoptions[page]){//遍历各个页面中的多个打包css文件配置
			let stream=gulp.src(config.cssoptions[page][file])
		           ...
		           .pipe(autoprefixer({//自动加前缀
        			...
        			.pipe(gulp.dest('./dist/'+page+'/css'))//输出到对应的文件
...
//					},//入口
//		    	output:{
//		    		filename:'[name].min.js'//[name]代表在entry中键名是什么
//		    	}
//		    }))
...
gulp.task("reload", function(){
	return gulp.src("./dist/**/*.html")//让所有的html文件都重新加载
		.pipe(connect.reload());
})
